...
class PID:
    ...
    def print_pid_equation(self):
        """Print the literal equation of the PID controller."""
        print("PID equation: ")
        print("Kp * err + Ki * integral + Kd * deriv_filt")

    def print_pid_function(self, measurement, setpoint):
        """Calculate and print the value of the PID function."""
        # Calculate error
        err = setpoint - measurement
        ...
        # Print the PID function value
        print("PID function value:", self.command)
...
class Tank:
    """ This class represents a tank with fluid level subject to inflow and outflow rates.
    """

    def __init__(self, A, Q_out, max_level, T):
        self.A = A                      # Cross-sectional area of the tank
        self.Q_out = Q_out              # Outflow rate of the tank
        self.max_level = max_level      # Maximum level of the tank
        self.T = T                      # Time step
        self.level = 0                  # Current level of the tank

    def Step(self, Q_in):
        """ Update the level of the tank based on the input flow rate Q_in.
        """
        # Calculate the derivative dh/dt using the input flow rate and the tank properties
        dh_dt = (Q_in - self.Q_out) / self.A

        # Update the level by integrating the derivative using the time step T
        self.level += dh_dt * self.T

        # Saturate tank level within the limits
